...
namespace MinimalNoteWebMVC.Controllers
{
    public class NotesController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var notes = await
                response.Content.ReadFromJsonAsync<List<Note>>();
        ...
        { 
            var client = _httpClientFactory.CreateClient("NotesApi");// oluşturduğumuz client'ı kullanarak apiye istek atıyoruz
            var response = await client.GetAsync($"notes/{id}");// apiye istek atıyoruz ve id'yi parametre olarak gönderiyoruz
            if (!response.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> Edit(int id, Note note)
        {
            ...
            var response = await client.PutAsJsonAsync($"notes/{id}", note);
